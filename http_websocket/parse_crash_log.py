...
class CrashParser:
    def __init__(self, cleardata, stacktrac, atospase, packagenames, curname, parser_wait_raw):
        ...
        self.request_raw = parser_wait_raw
    ...
    # OLD CODE. USE WEBSOCKET NOW. USELESS ANY MORE.
    ...
    def get_crash_info(self, pkgname):
        ...
        # data = self.get_crash_info_raw() # OLD CODE. USE WEBSOCKET NOW. USELESS ANY MORE.
        # Get application name
        print(self.request_raw)
        for k, v in enumerate(pkgname):
            if v in self.request_raw:
                self.cur_app_name = k
        # Split '\n' to get lines
        self.data_lines = self.request_raw.split('\n')
